...
if(isset($_POST['submit']))
  {
    ...
    $ownercontno=$_POST['MobileNumber'];
    $enteringtime=$_POST['enteringtime'];
    $query1 = "SELECT COUNT(*) as total FROM tblvehicle WHERE VehicleCategory='$catename' AND status=''"; 
    $ret1 = mysqli_query($con, $query1); $row = mysqli_fetch_assoc($ret1); 
    $ct = $row['total']; 
    
    $limit_query = mysqli_query($con, "SELECT `limit` FROM `tblcategory` WHERE VehicleCat='$catename'"); 
    $limit_row = mysqli_fetch_assoc($limit_query);
     $actual_limit = $limit_row['limit'];
    echo $ct;
    if ($ct < $actual_limit)
    {
    ...
    else{echo "<script>alert('The count $ct has reached or exceeded the limit $actual_limit.');</script>";}
...
while ($row = mysqli_fetch_array($ret)) {
    ...
    <div class="stat-icon dib flat-color-3">
        default : <?php echo $row['MobileNumber']; ?> 
    </div>
